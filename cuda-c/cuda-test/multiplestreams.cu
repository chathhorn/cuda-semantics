...
// Still need cudaEventElapsedTime, cudaGetDeviceCount, cudaDeviceProp,
// cudaFreeHost, cudaMallocHost
#include <stdio.h>
#include <cuda.h>
...
int main(int argc, char *argv[]) {
      ...
      init_array<<<blocks, threads, 0, streams[0]>>>(d_a, d_c);
      ...
      for(int k = 0; k < nreps; k++) {
            init_array<<<blocks, threads>>>(d_a, d_c);
      ...
      for(int k = 0; k < nreps; k++) {
            ...
            for(int i = 0; i < nstreams; i++)
                  init_array<<<blocks, threads, 0, streams[i]>>>(d_a + i * n / nstreams, d_c);
